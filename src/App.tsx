...
export default function App() {
  ...
  const [odds, setOdds] = useState<number>(48.5);
  const [rebate, setRebate] = useState<number>(0);
  const [activeView, setActiveView] = useState<'stats' | 'compound'>('stats');
  const [drawNumbers, setDrawNumbers] = useState<(number | null)[]>(Array(7).fill(null));
  ...
  const handleParse = (isNegative: boolean = false, customInput?: string) => {
    ...
    try {
      ...
      if (activeView === 'compound') {
        setCompoundRecords(prev => [ newRecord, ...prev ].slice(0, 50));
      } else {
        setFinanceRecords(prev => [ newRecord, ...prev ].slice(0, 50));
      }
  ...
  const handleExport = () => {
    try {
      ...
      const wsData = [
        ['用户中奖金额', '应该赔付用户多少金额'],
        ...exportData.map(d => [d.winningStake, d.payout])
      ];
      const ws = XLSX.utils.aoa_to_sheet(wsData);

      // Add comments to the first column (Winning Amount)
      exportData.forEach((d, i) => {
        const cellRef = XLSX.utils.encode_cell({ r: i + 1, c: 0 });
        
        if (d.fullRaw) {
          // SheetJS standard for comments
          // By default, comments are hidden in most Excel viewers unless 'visible: true' is set
          ws[cellRef].c = [{
            t: String(d.fullRaw).trim(),
            a: "录入原文"
          }];
          
          // Explicitly set hidden property for compatibility
  ...
  const formatModalResults = (input: string) => {
    ...
    try {
      if (activeView === 'compound') {
          ...
          segments.forEach((seg, idx) => {
            ...
            if (amountPerGroup > 0) {
              ...
              if (segmentCount > 0) {
                ...
                totalBet += subTotal;
                preview += `- ${type}: ${segmentNumbers.join(' | ')} (${segmentCount}组 x ${amountPerGroup}元 = ${subTotal}元)\n`;
              }
            }
          });
        ...
        return '格式错误，未识别到玩法或金额。格式如："二中二 05-19 10"';
      }

      const results = parseInput(input);
      if (results.length === 0) return '无法解析，请检查格式';
      
      let grandTotal = 0;
      let totalNumbers = 0;
      const lines = results.map(res => {
        const sortedNums = [...res.numbers].sort((a, b) => a - b);
        const numStr = sortedNums.map(n => n.toString().padStart(2, '0')).join('-');
        const count = res.numbers.length;
        const total = count * res.amount;
        grandTotal += total;
        totalNumbers += count;
        return `${numStr}\n共 ${count} 个号码，各 ${res.amount} 元 (总计 ${total} 元)`;
      });

      lines.push(`\n------------------\n本次录入总计：${grandTotal} 元 (共 ${totalNumbers} 个号码)`);
      ...
      <div className="max-w-7xl mx-auto space-y-4">
        {error && (
          <motion.div 
            ...
            className="bg-red-50 border-2 border-red-600 p-3 flex items-center justify-between shadow-[4px_4px_0px_0px_rgba(220,38,38,1)]"
          >
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-[#141414] pb-2 gap-2">
          {/* Global View Tabs */}
          <div className="flex gap-1 bg-white p-0.5 border border-[#141414] shadow-[2px_2px_0px_0px_rgba(20,20,20,1)] w-fit">
            <button 
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <span className="text-[9px] font-mono opacity-50 uppercase">Total Turnover</span>
              <span className="text-lg font-mono font-bold">¥{totalTurnover.toLocaleString()}</span>
            </div>
            <div className="h-6 w-[1px] bg-[#141414] opacity-20 mx-1 hidden md:block" />
          ...
          {activeView === 'stats' ? (
            <>
              ...
              <div className="lg:col-span-5 space-y-6">
                <section className="bg-white border border-[#141414] p-4 shadow-[4px_4px_0px_0px_rgba(20,20,20,1)] h-full">
                  ...
                  <div className="grid grid-cols-4 gap-x-0.5 gap-y-1.5">
                    {(() => {
                      const rows = 14; // Set to 14 rows as requested
                      const indices = [];
                      for (let r = 0; r < rows; r++) {
                        for (let c = 0; c < 4; c++) {
                          let num = null;
                          if (c < 3) {
                            if (r < 12) num = c * 12 + r + 1;
                          } else {
                            // Column 4
                            num = 36 + r + 1;
                            if (num > 49) num = null;
                      ...
                      return indices.map((num, idx) => {
                        ...
                        return (
                          <div 
                            key={num}
                            className={`flex items-center gap-1 py-1.5 transition-colors hover:bg-black/5 px-1 rounded ${isSpecial ? 'bg-yellow-50 ring-1 ring-yellow-100' : ''}`}
                          >
                            <div className="flex items-center gap-0.5 min-w-[48px]">
                            ...
                            <div className="flex-1 min-w-[36px] px-1 py-0.5 border border-[#141414] text-right text-xs font-mono font-bold bg-white shadow-[1px_1px_0px_0px_rgba(20,20,20,1)] text-[#141414]">
              ...
              <div className="lg:col-span-4 space-y-6">
                ...
                <section className="bg-white border border-[#141414] p-4 shadow-[4px_4px_0px_0px_rgba(20,20,20,1)]">
                  ...
                  <div className="flex flex-col gap-3">
                    <button
                      ...
                      className="w-full text-[#E4E3E0] py-4 font-mono text-sm font-bold hover:bg-opacity-90 transition-all active:translate-y-1 active:shadow-none flex items-center justify-center gap-2 bg-[#141414]"
                    >
                      <Plus size={18} />
                      录入下注 (RECORD)
                    </button>
                    <button
                      ...
                      className="w-full border-2 border-red-600 text-red-600 py-4 font-mono text-sm font-bold hover:bg-red-600 hover:text-white transition-all active:translate-y-1 active:shadow-none flex items-center justify-center gap-2"
                      title="将当前输入作为负数扣除"
                    >
                ...
                {/* History Section */}
                <section className="bg-white border border-[#141414] p-4 shadow-[4px_4px_0px_0px_rgba(20,20,20,1)] max-h-[400px] overflow-hidden flex flex-col">
              ...
              <div className="lg:col-span-3 space-y-6">
                <section className="bg-white border border-[#141414] p-4 shadow-[4px_4px_0px_0px_rgba(20,20,20,1)] flex flex-col h-full">
                  ...
                  <div className="flex-1 space-y-[1px] pr-2">
                    {(() => {
                      ...
                      .map(item => {
                        ...
                        return (
                          <div 
                            key={item.num} 
                            className={`py-0 px-1 border-b border-[#141414] flex items-center justify-between transition-colors ${item.risk < 0 ? 'bg-red-50' : 'bg-emerald-50'}`}
                            style={{ height: '18px' }}
                          >
                            <div className="flex items-center gap-1.5 leading-none">
                              <span className={`text-xs font-mono font-bold w-5 ${textColor}`}>{item.num.toString().padStart(2, '0')}</span>
                              <span className={`text-[11px] font-bold w-5 h-4 flex items-center justify-center bg-black/5 rounded-sm ${textColor}`}>{zodiac}</span>
                              <span className="text-[10px] font-mono font-bold text-[#141414]">注:¥{item.amount.toFixed(0)}</span>
                            </div>
                            <div className={`text-[10px] font-mono font-bold leading-none ${item.risk < 0 ? 'text-red-600' : 'text-emerald-600'}`}>
            ...
            <>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-col gap-2">
                      <button
                        ...
                        className="w-full bg-indigo-600 text-white py-3 font-mono text-sm font-bold hover:bg-indigo-700 transition-all active:translate-y-1 active:shadow-none flex items-center justify-center gap-2 shadow-[2px_2px_0px_0px_rgba(20,20,20,1)]"
                      >
                        <Plus size={18} />
                        录入复式
                      </button>
                      <button
                        ...
                        className="w-full border border-red-600 text-red-600 py-3 font-mono text-sm font-bold hover:bg-red-600 hover:text-white transition-all active:translate-y-1 active:shadow-none flex items-center justify-center gap-2"
                      >
                        <RotateCcw size={18} className="rotate-180" />
                        扣除复式
                      </button>
              ...
              <div className="lg:col-span-8 space-y-4">
                <section className="bg-white border border-[#141414] p-4 shadow-[4px_4px_0px_0px_rgba(20,20,20,1)] flex flex-col h-full min-h-[400px]">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      <History size={16} />
                    ...
                    <div className="text-right">
                      <span className="text-[9px] font-mono opacity-50 block uppercase">Compound Total</span>
                  ...
                  <div className="overflow-y-auto flex-1 space-y-4 pr-2">
                    {compoundRecords.filter(r => r.items.some(i => i.raw.includes('中') || i.raw.includes('特碰'))).length === 0 ? (
                      ...
                      compoundRecords.filter(r => r.items.some(i => i.raw.includes('中') || i.raw.includes('特碰'))).map(record => {
                        const regularDraw = drawNumbers.slice(0, 6).filter((n): n is number => n !== null);
                        ...
                        return (
                          <div key={record.id} className={`border-2 p-4 relative group transition-all ${hasAnyWin ? 'bg-yellow-50 border-yellow-500 shadow-[4px_4px_0px_0px_rgba(234,179,8,1)]' : 'bg-[#F9F9F7] border-[#141414]'}`}>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
            <motion.div 
              ...
              className="bg-[#E4E3E0] border-2 border-[#141414] p-6 max-w-2xl w-full shadow-[12px_12px_0px_0px_rgba(20,20,20,1)] flex flex-col gap-4"
            >
              <div 
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <div className="w-full h-32 p-4 font-mono text-sm border-2 border-[#141414] bg-[#F5F5F0] overflow-y-auto whitespace-pre-wrap break-all">
                    {formatModalResults(modalInputValue)}
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-4 mt-2">
                <button 
                  ...
                  className="flex-1 bg-[#141414] text-[#E4E3E0] py-4 font-mono text-lg font-bold hover:bg-opacity-90 transition-all active:translate-y-1 active:shadow-none disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  确认录入 (CONFIRM)
                </button>
                <button 
        ...
        {showResetConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
            <motion.div 
              ...
              className="bg-white border-2 border-[#141414] p-6 max-w-sm w-full shadow-[8px_8px_0px_0px_rgba(20,20,20,1)]"
            >
